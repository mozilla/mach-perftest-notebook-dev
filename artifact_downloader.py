...
def artifact_downloader_parser():
    ...
    parser.add_argument(
        "--platform", type=str, default="test-linux64-ccov", help="Platform to obtain data from.",
    )
    parser.add_argument(
        "--download-failures",
        action="store_true",
...
def log(msg):
    ...
    print("[%s][INFO] %s/%s %s -  %s" % (val, str(CURR_TASK + 1), str(TOTAL_TASKS), pc, msg))
...
def warning(msg):
    ...
    val = time.strftime("%H:%M:%S", time.gmtime(elapsed_time))
    pc = round((CURR_TASK / TOTAL_TASKS) * 100, 1) if TOTAL_TASKS else 0
    print("[%s][WARNING] %s/%s %s -  %s" % (val, str(CURR_TASK + 1), str(TOTAL_TASKS), pc, msg))
...
def get_tasks_in_group(group_id):
    reply = get_json(
        "https://queue.taskcluster.net/v1/task-group/" + group_id + "/list", {"limit": "200",},
    )
    tasks = reply["tasks"]
...
def download_artifact(task_id, artifact, output_dir):
    ...
    while tries < RETRY:
        try:
            ...
            log("Failed to get data from %s: %s - %s" % (url_data, e.__class__.__name__, e))
            if tries < RETRY:
                tries += 1
...
def artifact_downloader(
    ...
    for task in tasks:
        ...
        log("Found %s with suite-name: %s" % (task["task"]["metadata"]["name"], test_name))
        ...
        if all_tasks or download_this_task:
            ...
            data_dir = os.path.join(
                os.path.join(grcov_dir, (artifact_to_get.replace(".", "")) + "_data")
            )
            ...
            def get_artifacts(
                ...
                if ffound:
                    ...
                    taskid_to_file_map[task_id] = os.path.join(data_dir, str(test_counter))
    ...
    if FAILED:
        log("Tasks the failed to have their artifact downloaded: %s" % "\n\t".join(FAILED))
