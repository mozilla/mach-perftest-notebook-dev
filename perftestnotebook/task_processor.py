...
def get_task_data_paths(
    ...
    all_suites = [f for f in os.listdir(run_dir) if os.path.isdir(os.path.join(run_dir, f))]
    ...
    for suite in all_suites:
        ...
        all_dirs = [f for f in os.listdir(suite_dir) if os.path.isdir(os.path.join(suite_dir, f))]
        suite_data_dir = None
        for d in all_dirs:
        ...
        all_files = sorted_nicely(
            [file for file in all_files if artifact and artifact in os.path.split(file)[-1]]
        )
